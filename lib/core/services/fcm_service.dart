...
class FcmService {
  ...
  Future<void> _initOnce() async {
    ...
    try {
      // A transient Firebase startup failure must not permanently mark FCM as
      // initialized. Leave _initialized false so a later explicit init can
      // retry after connectivity/plugin startup has recovered.
      ...
      // Once transport/listeners are ready, token or initial-message sync
      // failures are recoverable via token refresh/syncRegistration and must not
      // cause a later init call to attach duplicate stream listeners.
      _initialized = true;
