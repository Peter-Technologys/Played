...
class GoogleAccountService {
  ...
  Future<AuthResult> signInAndAuthenticate() async {
    ...
    try {
      ...
      debugPrint(
        '[GoogleAccount] sign-in platform failure: code=${e.code}, '
        'detailsType=${e.details.runtimeType}',
      );
      return AuthResult(ok: false, error: _friendlyPlatformError(e));
    } catch (e) {
  ...
  Future<void> backupToDrive() async {
    ...
    if (token == null) {
      throw StateError('Google Drive permission is required for backup.');
    }
  ...
  Future<int> restoreFromDrive() async {
    final token = await _freshDriveToken();
    if (token == null) {
      throw StateError('Google Drive permission is required for restore.');
    }
  ...
  Future<void> deleteDriveBackup() async {
    final token = await _freshDriveToken();
    if (token == null) {
      throw StateError('Google Drive permission is required to delete the backup.');
    }
