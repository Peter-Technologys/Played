...
class WebShareMirrorService {
  ...
  Future<void> _html(HttpRequest req) async {
    // JS uses ES5 string concatenation (no backtick template literals) so that
    // Dart never sees a bare \${} and raises interpolation parse errors.
...
<script>
var all=[];
async function load(){try{var r=await fetch("/api/media-list");all=await r.json();document.getElementById("cnt").textContent=all.length+" files";render(all);}catch(e){document.getElementById("grid").innerHTML="<div class=\"empty\">Error loading library.</div>";}}
function render(items){var g=document.getElementById("grid");if(!items.length){g.innerHTML="<div class=\"empty\">No files.</div>";return;}
g.innerHTML=items.map(function(f){var e=encodeURIComponent(f.path);var ic=f.isVideo?"&#127916;":"&#127925;";
var sb=f.isVideo?"<button class=\"btn s\" onclick=\"stream(\'" +e+ "\')\">&nbsp;&#9654; Stream</button>":"";
return"<div class=\"card\"><div class=\"thumb\">" +ic+ "</div><div class=\"body\"><div class=\"title\" title=\"" +f.title+ "\">" +f.title+ "</div><div class=\"meta\">" +f.size+ "</div><div class=\"actions\">" +sb+ "<button class=\"btn d\" onclick=\"dl(\'" +e+ "\')\">&darr; Download</button></div></div></div>";}).join("");}
function filter(){var q=document.getElementById("search").value.toLowerCase();render(q?all.filter(function(f){return f.title.toLowerCase().includes(q);}):all);}
function stream(e){document.getElementById("vp").src="/api/stream?path="+e;document.getElementById("modal").classList.add("open");}
function dl(e){var a=document.createElement("a");a.href="/api/stream?path="+e;a.download="";a.click();}
function close2(){var v=document.getElementById("vp");v.pause();v.src="";document.getElementById("modal").classList.remove("open");}
document.getElementById("modal").addEventListener("click",function(e){if(e.target===document.getElementById("modal"))close2();});
load();
</script></body></html>''';
