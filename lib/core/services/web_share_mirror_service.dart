...
class WebShareMirrorService {
  ...
  Future<void> _html(HttpRequest req) async {
    // JS template literals use \${} — escape every \$ so Dart does not
    // treat them as string interpolation inside this triple-quoted string.
    final html = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">\n'
        ...
        '<div id="modal"><button id="mc" onclick="close2()">&times;</button><video id="vp" controls autoplay></video></div>\n'
        ...
        r"g.innerHTML=items.map(f=>{const e=encodeURIComponent(f.path);const ic=f.isVideo?'\uD83C\uDFAC':'\uD83C\uDFB5';"
        r"return`<div class=""card""><div class=""thumb"">${ic}</div><div class=""body""><div class=""title"" title=""${f.title}"">${f.title}</div><div class=""meta"">${f.size}</div><div class=""actions"">${f.isVideo?`<button class=""btn s"" onclick=""stream('${e}')"">&#9654; Stream</button>`:''}<button class=""btn d"" onclick=""dl('${e}')"">\u2B07 Download</button></div></div></div>`;}).join('');}"
