...
void main() {
  ...
  test('manual approval preserves release-integrity checks before publication', () {
    final workflow = File('.github/workflows/release.yml').readAsStringSync();

    final approvalIndex = workflow.indexOf('Verify explicit release approval');
    final signerIndex = workflow.indexOf('verify --verbose --print-certs');
    final publishIndex = workflow.indexOf('- name: Publish Otya APKs');

    expect(approvalIndex, greaterThanOrEqualTo(0));
    expect(signerIndex, greaterThan(approvalIndex));
    expect(publishIndex, greaterThan(signerIndex));
    expect(workflow, contains('Check release version matches app'));
